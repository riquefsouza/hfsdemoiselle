...
public class AssinaturaFrameJNLP extends JFrame {

	private static final long serialVersionUID = 1L;

	private static final Logger logger = Logger
			.getLogger(AssinaturaFrameJNLP.class.getName());
	
	public AssinaturaFrameJNLP(String urlBase) {
		super("Assinador Digital");
		
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		AssinaturaPanel assinaturaPanel = new AssinaturaPanel(false, urlBase);
			...
			public void run() {
				...
				if (urlBase!=null){
					if (!urlBase.isEmpty()){
						...
						new AssinaturaFrameJNLP(urlBase);
					}
				}
			}
		});
